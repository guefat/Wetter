...
tof = VL53L1X.VL53L1X(i2c_bus=1, i2c_address=0x29)  #Abstandssensor
tof.open()
tof.start_ranging(2)
...
volt = gray = Vmax = Vmaxx = Counter = rainamount = 0
Tmin = Tminn = 50
Tmax = Tmaxx = -50
c1=-8.784695
c2=1.61139411
...
def readSnow():                         #Funktion Schneehöhe (VL53L1X)
    distance_neu = 0
    for n in range(100):
      distance = tof.get_distance()
      sleep(0.1)
      distance_neu = distance_neu + distance
    distance_neu = distance_neu/100
    distance_neu = (distance_neu)
    return distance_neu
...
def readWindSpeed():                    #Funktion Windgeschwindigkeit (Adafruit Anemometer)
    ...
    UwindSpeed = UwindSpeed - 0.3       #0.3 Volt entspricht 0 m/s  ???
    if UwindSpeed < 0:
        UwindSpeed = 0
    V=UwindSpeed*32.4/1.5             # (/1.6) Umrechnung Spannung auf Geschwindigkeit m/s 
...
while True:
        ...
        snowlevel = readSnow()          #Auslesen der Schneehöhe (VL53L1X)
        snowlevel = 0                    #abs(snowlevel/10-215)
        ...
        #print(gray)
        rainamount = Counter*0.2794     #Regenmenge in mm         
